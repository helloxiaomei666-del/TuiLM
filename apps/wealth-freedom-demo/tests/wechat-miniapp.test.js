...
test("miniapp final copy avoids prototype wording in visible surfaces", () => {
  ...
  assert.doesNotMatch(readFileSync(path.join(miniRoot, "pages/overview/overview.wxml"), "utf8"), /MVP|Demo|刷新估值 Demo|模拟截图识别|>清除</);
...
test("overview page uses V2 status-first structure without dashboard noise", () => {
  const overviewWxml = readFileSync(path.join(miniRoot, "pages/overview/overview.wxml"), "utf8");
  const overviewWxss = readFileSync(path.join(miniRoot, "pages/overview/overview.wxss"), "utf8");
  ...
  assert.match(overviewWxml, /class="overview-brand"/);
  assert.match(overviewWxml, /今天，你退休了吗？/);
  assert.match(overviewWxml, /本月更新：/);
  assert.match(overviewWxml, /总退休率/);
  assert.match(overviewWxml, /overviewView\.phaseLabel/);
  assert.match(overviewJs, /起步期/);
  assert.match(overviewJs, /主要依赖工资收入/);
  assert.match(overviewJs, /下一步先提高被动收入覆盖率/);
  assert.match(overviewWxml, /被动收入覆盖率/);
  assert.match(overviewWxml, /现金安全垫/);
  assert.match(overviewWxml, /总资产进度/);
  assert.match(overviewWxml, /查看我的退休路线/);
  assert.match(overviewWxml, /结果仅基于填写数据估算，不构成投资建议。/);
  assert.match(overviewWxml, /查看说明/);
  assert.match(overviewWxml, /退休路线/);
  assert.match(overviewWxml, /routePreview\.headline/);
  assert.match(overviewJs, /预计第 .* 年接近目标|当前假设下暂未看到接近目标|已超过当前设定目标/);
  assert.match(overviewWxml, /routePreview\.nodes/);
  assert.match(overviewJs, /现在/);
  assert.match(overviewJs, /10年/);
  assert.match(overviewWxml, /routePreview\.gapText/);
  assert.match(overviewJs, /距离目标还差|已超过当前设定目标/);
  assert.match(overviewWxml, /查看路线详情/);
  assert.match(overviewWxml, /本月建议/);
  assert.match(overviewWxml, /先补充资产明细/);
  assert.match(overviewWxml, /当前最建议完善/);
  assert.match(overviewWxml, /资产数据/);
  assert.match(overviewWxml, /保障数据/);
  assert.match(overviewWxml, /去完善基础数据/);
  assert.match(overviewWxml, /class="progress-card"/);
  assert.match(overviewWxml, /class="route-preview-card"/);
  assert.match(overviewWxml, /class="advice-card"/);
  assert.equal((overviewWxml.match(/class="stat-item"/g) || []).length, 3);
  assert.match(overviewWxml, /displayUpdatedDate/);
  assert.match(overviewWxml, /retirementIndexText/);
  assert.match(overviewWxml, /retirementIndexAvailable/);
  assert.match(overviewWxml, /retirementIndexCompleteness/);
  assert.match(overviewWxml, /passiveIncomeCoverageText/);
  assert.match(overviewWxml, /cashSafetyRunwayText/);
  assert.match(overviewWxml, /totalAssetProgressText/);
  ...
  assert.match(overviewWxml, /openLegal/);
  assert.match(overviewJs, /routePreview/);
  assert.doesNotMatch(overviewWxml, />隐私与免责声明</);
  assert.doesNotMatch(overviewWxml, /下一步行动/);
  assert.doesNotMatch(overviewWxml, /补充资产<\/text>/);
  assert.doesNotMatch(overviewWxml, /完善保障<\/text>/);
  assert.doesNotMatch(overviewWxml, /查看路径明细/);
  assert.doesNotMatch(overviewWxml, /检查拖累项/);
  assert.doesNotMatch(overviewWxml, /拖累项/);
  assert.doesNotMatch(overviewWxml, /dragTotalText|dragCountText/);
  assert.doesNotMatch(overviewWxml, /class="edit-panel"/);
  assert.doesNotMatch(overviewWxml, /编辑基础数据/);
  assert.doesNotMatch(overviewWxml, /isEditingProfile|showAdvancedProfile/);
  assert.doesNotMatch(overviewWxml, /toggleEditPanel|toggleAdvancedProfile/);
  assert.doesNotMatch(overviewWxml, /manual|source|示例初始/);
  assert.doesNotMatch(overviewWxml, /open-type="switchTab"/);
  assert.doesNotMatch(overviewWxml, /class="action-grid"/);
  assert.doesNotMatch(overviewWxml, /class="action-card/);
  assert.doesNotMatch(overviewWxss, /font-weight:\s*(8|9)\d\d/);
  assert.doesNotMatch(overviewWxss, /\.action-card/);
  assert.doesNotMatch(overviewWxss, /\.edit-panel/);
