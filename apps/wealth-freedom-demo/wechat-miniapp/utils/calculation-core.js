(function attachWealthCalculation(global) {
  ...
  function normalizeHolding(holding = {}, options = {}) {
    ...
    const categoryConfig = assetCategories[type] || {};

    return {
      ...holding,
      type,
      instrument: holding.instrument || (type === "stock" ? "fund" : type === "bond" ? "bondFund" : "commodityFund"),
      name: holding.name || categoryConfig.label || "未命名资产",
      ...
      source: holding.source || "手动录入",
      apiProvider: holding.apiProvider || categoryConfig.apiProvider,
  ...
  function simulate(values, overrides = {}) {
    ...
    return {
      ...config,
      months: reachedMonth === null || reachedMonth === undefined ? 720 : reachedMonth,
  ...
  function getSecurityAccountTotal(accounts = {}) {
    return [
      accounts.pension && accounts.pension.balance,
      accounts.housingFund && accounts.housingFund.balance,
      accounts.supplementalHousingFund && accounts.supplementalHousingFund.balance,
      accounts.enterpriseAnnuity && accounts.enterpriseAnnuity.balance,
      accounts.occupationalAnnuity && accounts.occupationalAnnuity.balance,
    ].reduce((sum, value) => sum + numberOr(value, 0), 0);
  }

  function getSecurityRetirementIncomeMonthly(accounts = {}) {
    return [
      accounts.pension && accounts.pension.estimatedMonthlyBenefit,
      accounts.enterpriseAnnuity && accounts.enterpriseAnnuity.estimatedMonthlyBenefit,
      accounts.occupationalAnnuity && accounts.occupationalAnnuity.estimatedMonthlyBenefit,
    ].reduce((sum, value) => sum + numberOr(value, 0), 0);
  }

  function getHousingLoanOffsetMonthly(accounts = {}, mortgage = 0) {
    const rawOffset =
      numberOr(accounts.housingFund && accounts.housingFund.loanOffsetMonthly, 0) +
      numberOr(accounts.supplementalHousingFund && accounts.supplementalHousingFund.loanOffsetMonthly, 0);
  ...
  function getSecuritySupport(values, baseResult, securityAccounts = {}, pensionYearsTarget = 20) {
    ...
    return {
      ...
      pensionProgress: clamp(
        (numberOr(securityAccounts.pension && securityAccounts.pension.yearsPaid, 0) / pensionYearsTarget) * 100,
        0,
        100
      ),
    };
